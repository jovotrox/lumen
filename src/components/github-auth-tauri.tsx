...
export function TauriSignInButton(props: ButtonProps) {
  ...
  const startDeviceFlow = async () => {
    ...
    try {
      // Request device code using Tauri's HTTP client (no CORS restrictions)
...
async function getUser(token: string) {
  const userResponse = await tauriFetch("https://api.github.com/user", {
    headers: {
  ...
  const emailResponse = await tauriFetch("https://api.github.com/user/emails", {
    headers: {
      Authorization: `Bearer ${token}`,
    },
  })
